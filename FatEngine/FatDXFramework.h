...
// --- High level user interface ---
        ...
        int     CreateTextureFromDds (const std::wstring& filename);




        // --- Render ---
        void SetRenderTarget (shared_ptr<Texture> targettexture, const XMFLOAT4& clearColor);
        void DrawStaticObjectsSets (int* noObjects, const int& size);
        void RenderToTarget ();
        void Render (int* noObjects, const int& size);
        ...
        struct RootParameter
        {
            ...
            D3D12_GPU_VIRTUAL_ADDRESS   ConstantBuffer = -1;
        };

        class MatrixBuffer
        {
        public:
            MatrixBuffer (std::shared_ptr<FatDXFramework> fx) {
                for (int i = 0; i < 4; i++) {
                    m_Matrix.push_back (XMMatrixIdentity ());
                }
                m_ConstantBuffer = fx->CreateUploadBuffer (256);
            }

            XMMATRIX& operator[](int i) {
                if (i < 4) {
                    return m_Matrix[i];
                }
            }
            ...
            void Upload () {
                XMFLOAT4X4* mdata;
                m_ConstantBuffer->Map (0, nullptr, reinterpret_cast<void**> (&mdata));
                XMStoreFloat4x4 (&mdata[0], XMMatrixTranspose (m_Matrix[0]));
                m_ConstantBuffer->Unmap (0, nullptr);
            }

        private:
            std::vector<XMMATRIX> m_Matrix;
            ComPtr<ID3D12Resource> m_ConstantBuffer;
        };


        bool CreateCbv (ComPtr<ID3D12Resource> buffer);
        ...
        int CreateGeometry ();
        void AddDataToGeometry (const int& gidx, const std::vector<float>& gdata);
